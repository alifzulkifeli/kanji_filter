...
const observer = new MutationObserver((mutations) => {mutations.forEach((mutation) => {for (const node of mutation.addedNodes) {if (node.className == "player-timedtext-text-container") {const text = node.innerText.replaceAll("\n", "");GM_setClipboard(text);}}})});observer.observe(document, { subtree: true, childList: true });
// Get known words
